...
export function Header() {
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors duration-500 border-b border-border bg-background/90 backdrop-blur-md`}
    >
      <div className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-5 lg:px-10">
          ...
          {NAV.map((item) => (
            <a
              ...
              className={`link-sweep text-sm transition-colors ${
                scrolled ? "text-black hover:text-black/70" : "text-black hover:text-black/70"
              }`}
            >
              {item}
            </a>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className={`h-[1.5px] w-5 transition-all duration-300 ${open ? "bg-black translate-y-[7.5px] rotate-45" : "bg-black"}`} />
            <span className={`h-[1.5px] w-5 transition-all duration-300 ${open ? "opacity-0" : "bg-black"}`} />
            <span className={`h-[1.5px] w-5 transition-all duration-300 ${open ? "bg-black -translate-y-[7.5px] -rotate-45" : "bg-black"}`} />
          </button>
